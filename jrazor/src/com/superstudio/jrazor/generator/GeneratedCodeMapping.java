...
public final class GeneratedCodeMapping {
	...
	public GeneratedCodeMapping(int startOffset, int startLine, int startColumn, int startGeneratedColumn,
			int codeLength) {
		this((Integer) startOffset, startLine, startColumn, startGeneratedColumn, codeLength);
	}

	private GeneratedCodeMapping(Integer startOffset, int startLine, int startColumn, int startGeneratedColumn,
			int codeLength) {
		// this();
		if (startLine < 0) {
			// throw new ArgumentOutOfRangeException("startLine", String.format(
			// CommonResources.getArgument_Must_Be_GreaterThanOrEqualTo(),
			// "startLine", "0"));
		}
		if (startColumn < 0) {
		...
		if (startGeneratedColumn < 0) {
			// throw new ArgumentOutOfRangeException("startGeneratedColumn",
			// String.format(
			// CommonResources.getArgument_Must_Be_GreaterThanOrEqualTo(),
			// "startGeneratedColumn", "0"));
		}
		if (codeLength < 0) {
	...
	public boolean equals(Object obj) {
		...
		return getCodeLength() == other.getCodeLength() && getStartColumn() == other.getStartColumn()
				&& getStartGeneratedColumn() == other.getStartGeneratedColumn()
				&& getStartLine() == other.getStartLine() && (getStartOffset() == null || other.getStartOffset() == null
						|| getStartOffset().equals(other.getStartOffset()));
	}

	@Override
	public String toString() {
		return String.format("(%s, %d, %d) -> (?, %d) [%d]",
				getStartOffset() == null ? "?" : getStartOffset().toString(), getStartLine(), getStartColumn(),
				getStartGeneratedColumn(), getCodeLength());
	}

	@Override
	public int hashCode() {
		return HashCodeCombiner.Start().Add(getCodeLength()).Add(getStartColumn()).Add(getStartGeneratedColumn())
				.Add(getStartLine()).Add(getStartOffset()).getCombinedHash();
	}
